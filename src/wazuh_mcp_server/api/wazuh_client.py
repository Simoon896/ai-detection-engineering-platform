...
class WazuhClient:
    ...
    # ------------------------------------------------------------------
    # Ruleset / decoder file management + logtest (false-positive tuning)
    # ------------------------------------------------------------------
    async def _raw_api_call(
        ...
        *,
        ...
        """Authenticated request that can return raw text (e.g. raw XML rule files)
        or send a raw body (e.g. PUT rule file). Handles a single 401 re-auth.

        Unlike _request(), this does not require a JSON 'data' envelope, so it works
        for ?raw=true XML responses and octet-stream uploads.
        """
        async with self._rate_limiter:
            ...
            async def _do() -> httpx.Response:
                resp = await self.client.request(
                    method, url, headers=_build_headers(), params=params, content=content
                )
            ...
            try:
                ...
                if e.response.status_code == 401:
                    ...
                    async with self._auth_lock:
                        if self.token is None or self.token == stale:
                            self.token = None
                            await self._authenticate()
                    response = await _do()
                else:
                    raise ValueError(
                        f"Wazuh API error for {endpoint}: HTTP {e.response.status_code} - {e.response.text[:500]}"
                    )
    ...
    async def list_rule_files(self, **params) -> Dict[str, Any]:
        """List ruleset files available on the manager (GET /rules/files)."""
        return await self._request("GET", "/rules/files", params=params)

    async def get_rule_file(self, filename: str) -> str:
        """Return the raw XML content of a ruleset file (GET /rules/files/{filename}?raw=true)."""
        return await self._raw_api_call("GET", f"/rules/files/{filename}", params={"raw": "true"}, parse="text")

    async def update_rule_file(self, filename: str, content: str, overwrite: bool = True) -> Dict[str, Any]:
        """Upload/replace a ruleset file (PUT /rules/files/{filename}). WRITE — gate behind approval."""
        return await self._raw_api_call(
            "PUT",
    ...
    async def list_decoder_files(self, **params) -> Dict[str, Any]:
        """List decoder files available on the manager (GET /decoders/files)."""
        return await self._request("GET", "/decoders/files", params=params)

    async def get_decoder_file(self, filename: str) -> str:
        """Return the raw XML content of a decoder file (GET /decoders/files/{filename}?raw=true)."""
        return await self._raw_api_call("GET", f"/decoders/files/{filename}", params={"raw": "true"}, parse="text")

    async def update_decoder_file(self, filename: str, content: str, overwrite: bool = True) -> Dict[str, Any]:
        """Upload/replace a decoder file (PUT /decoders/files/{filename}). WRITE — gate behind approval."""
        return await self._raw_api_call(
            "PUT",
    ...
    async def run_logtest(self, event: str, log_format: str = "syslog", location: str = "stdin") -> Dict[str, Any]:
        """Run the Wazuh logtest engine against a log line (PUT /logtest).

        Returns the matched decoder + rule (id, level, description, groups) so a proposed
        rule/tuning can be validated against true-positive and false-positive samples.
        Note: logtest evaluates the manager's *currently loaded* ruleset, so deploy a
        proposed change (update_rule_file + wazuh_restart) before testing it.
        """
