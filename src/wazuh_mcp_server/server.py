...
def _validate_ruleset_filename(filename: Any) -> str:
    """Validate a rule/decoder filename: bare basename, .xml, no path traversal."""
    import re
    ...
    if "/" in filename or "\\" in filename or ".." in filename:
        raise ValueError("Invalid filename: provide a bare file name (no path), e.g. 'local_rules.xml'")
    if not re.match(r"^[A-Za-z0-9._-]+\.xml$", filename):
        raise ValueError("Invalid filename: must match [A-Za-z0-9._-]+.xml")
    if len(filename) > 255:
        raise ValueError("Filename too long")
    return filename


def _validate_xml_content(content: Any, max_bytes: int = 1_000_000) -> str:
    """Lightweight validation of rule/decoder XML content before upload."""
    import re

    if not content or not isinstance(content, str):
        raise ValueError("Parameter 'content' (XML) is required")
    if len(content.encode("utf-8")) > max_bytes:
        raise ValueError(f"Content too large (> {max_bytes} bytes)")
    if not content.lstrip().startswith("<"):
        raise ValueError("Content does not look like XML (must start with '<')")
    for match in re.finditer(r'<rule\s+id="(\d+)"', content, flags=re.IGNORECASE):
        rule_id = int(match.group(1))
        if rule_id > 999_999:
            raise ValueError(
                f"Invalid rule id {rule_id}: Wazuh requires integer rule IDs with at most 6 digits (<= 999999)"
            )
    return content
...
async def handle_tools_list(params: Dict[str, Any], session: MCPSession) -> Dict[str, Any]:
    ...
    tools = [
        ...
        # Detection Engineering / false-positive tuning tools (rules, decoders, logtest)
        {
            "name": "list_rule_files",
            "description": "List Wazuh ruleset files on the manager (e.g. local_rules.xml). Read-only.",
            "inputSchema": {
                "type": "object",
                "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 500}},
                "required": [],
            },
        },
        {
            "name": "get_rule_file",
            "description": "Get the raw XML content of a Wazuh ruleset file. Read-only.",
            "inputSchema": {
                "type": "object",
                "properties": {"filename": {"type": "string", "description": "Rule file name, e.g. 'local_rules.xml'"}},
                "required": ["filename"],
            },
        },
        {
            "name": "update_rule_file",
            "description": "[WRITE] Create or replace a Wazuh ruleset file with new XML (use for false-positive tuning). Requires approval. After updating, restart the manager (wazuh_restart target='manager') and validate with run_logtest.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "filename": {"type": "string", "description": "Rule file name, e.g. 'local_rules.xml'"},
                    "content": {"type": "string", "description": "Full XML content of the rule file"},
        ...
        {
            "name": "list_decoder_files",
            "description": "List Wazuh decoder files on the manager. Read-only.",
            "inputSchema": {
                "type": "object",
                "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 500}},
                "required": [],
        ...
        {
            "name": "get_decoder_file",
            "description": "Get the raw XML content of a Wazuh decoder file. Read-only.",
            "inputSchema": {
                "type": "object",
                "properties": {"filename": {"type": "string", "description": "Decoder file name, e.g. 'local_decoder.xml'"}},
                "required": ["filename"],
            },
        },
        {
            "name": "update_decoder_file",
            "description": "[WRITE] Create or replace a Wazuh decoder file with new XML. Requires approval. Restart the manager to apply.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "filename": {"type": "string", "description": "Decoder file name, e.g. 'local_decoder.xml'"},
                    "content": {"type": "string", "description": "Full XML content of the decoder file"},
                    "overwrite": {"type": "boolean", "default": True},
                },
                "required": ["filename", "content"],
            },
        ...
        {
            "name": "run_logtest",
            "description": "Test a raw log line against the manager's currently loaded ruleset (Wazuh logtest). Returns the matched decoder and rule (id, level, description, groups). Use to confirm a false positive is suppressed while a true positive still alerts.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "event": {"type": "string", "description": "Raw log line to test"},
                    "log_format": {"type": "string", "default": "syslog", "description": "Wazuh log format (syslog, json, eventchannel, ...)"},
                    "location": {"type": "string", "default": "stdin", "description": "Log location label"},
                },
                "required": ["event"],
            },
        },
...
async def handle_tools_call(params: Dict[str, Any], session: MCPSession) -> Dict[str, Any]:
    ...
    try:
        ...
        # Detection Engineering / false-positive tuning tools
        elif tool_name == "list_rule_files":
            limit = validate_limit(arguments.get("limit"), max_val=1000) if arguments.get("limit") is not None else 500
            result = await wazuh_client.list_rule_files(limit=limit)
            _success = True
            return _tool_result(f"Wazuh Rule Files:\n{json.dumps(result, indent=2, default=str)}")

        elif tool_name == "get_rule_file":
            filename = _validate_ruleset_filename(arguments.get("filename"))
            content = await wazuh_client.get_rule_file(filename)
            _success = True
            return _tool_result(f"Rule file {filename}:\n{content}")

        elif tool_name == "update_rule_file":
            filename = _validate_ruleset_filename(arguments.get("filename"))
            content = _validate_xml_content(arguments.get("content"))
            overwrite = validate_boolean(arguments.get("overwrite"), default=True, param_name="overwrite")
            result = await wazuh_client.update_rule_file(filename, content, overwrite=overwrite)
            _success = True
            return _tool_result(f"Updated rule file {filename}:\n{json.dumps(result, indent=2, default=str)}")

        elif tool_name == "list_decoder_files":
            limit = validate_limit(arguments.get("limit"), max_val=1000) if arguments.get("limit") is not None else 500
            result = await wazuh_client.list_decoder_files(limit=limit)
            _success = True
            return _tool_result(f"Wazuh Decoder Files:\n{json.dumps(result, indent=2, default=str)}")

        elif tool_name == "get_decoder_file":
            filename = _validate_ruleset_filename(arguments.get("filename"))
            content = await wazuh_client.get_decoder_file(filename)
            _success = True
            return _tool_result(f"Decoder file {filename}:\n{content}")

        elif tool_name == "update_decoder_file":
            filename = _validate_ruleset_filename(arguments.get("filename"))
            content = _validate_xml_content(arguments.get("content"))
            overwrite = validate_boolean(arguments.get("overwrite"), default=True, param_name="overwrite")
            result = await wazuh_client.update_decoder_file(filename, content, overwrite=overwrite)
            _success = True
            return _tool_result(f"Updated decoder file {filename}:\n{json.dumps(result, indent=2, default=str)}")

        elif tool_name == "run_logtest":
            event = arguments.get("event")
            if not event or not isinstance(event, str):
                raise ValueError("Parameter 'event' (raw log line) is required")
            validate_input(event, max_length=20000)
            log_format = str(arguments.get("log_format", "syslog"))
            location = str(arguments.get("location", "stdin"))
            result = await wazuh_client.run_logtest(event, log_format=log_format, location=location)
            _success = True
