...
class SecurityValidator:
    ...
    def validate_request(self, request: Request, body: Optional[str] = None) -> tuple[bool, Optional[str]]:
        ...
        # Check body content.
        # The MCP JSON-RPC endpoints carry tool arguments that legitimately
        # include Wazuh ruleset/decoder XML - e.g. "$(field)" substitution,
        # "<!-- ... -->" comments, and regex - which false-positive against the
        # generic command-injection/XSS patterns. Those tool inputs are validated
        # in the tool handlers (filename/XML/scope/approval), so skip the generic
        # body scan for MCP endpoints only; all other surfaces are still scanned.
        _mcp_endpoints = {"/mcp", "/sse", "/"}
        scan_body = request.url.path not in _mcp_endpoints
        if scan_body and body and self._contains_suspicious_pattern(body):
